...
def control_robot():
    ...
    # You can expand this for more actions (lights, fans, etc.)
    if action == "move" and direction in ["forward", "backward", "left", "right", "stop"]:
        # Call your robot movement logic here
        # Example: engine.move_robot(direction)
        try:
            result = None
            if hasattr(engine, "move_robot"):
